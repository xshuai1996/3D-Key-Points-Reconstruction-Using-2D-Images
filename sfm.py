...
class SFMSolver(object):
    ...
    def detect_and_match_feature(self, img1, img2):
        ...
        p1, p2 = np.float32(p1).reshape(-1, 1, 2), np.float32(p2).reshape(-1, 1, 2)
    ...
    def compute_pose(self, p1, p2, E):
        ...
        """
        # TODO: step 4
        _, R, trans, _ = cv2.recoverPose(E, p1, p2)
        return R, trans
...
def main():
    ...
    img_pattern = './dataset/rdimage.???.ppm'
    intrinsic = intrinsic_reader('./dataset/intrinsics.txt')
    output_dir = './output'
